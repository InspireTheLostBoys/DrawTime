  ...
  <div>
    <v-layout row justify-center>
      <v-dialog v-model="dialog" persistent max-width="600">
        <v-card>
          <v-toolbar flat :color="type" dark>
          ...
          <v-card-text class="mt-2" flat>
              <v-text-field v-model="text" :label="description" outlined>

              </v-text-field>
          </v-card-text>
          <v-card-actions style="text-align: center; display: block;">
            <v-btn color="success darken-1" @click.native="returnValue(text)">submit</v-btn>
...
export default {
  ...
  methods: {
    show(type, title, description, afterRuturn) {
      let self = this;
      self.type = type;
      self.title = title;
